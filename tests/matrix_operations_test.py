...
class TestMatrixOperations(unittest.TestCase):
    ...
    def test_compress_to_96_already_compressed(self):
        ...
        df = pd.DataFrame({"MutationType": helpers.MUTATION_LIST, "Value": [1] * len(helpers.MUTATION_LIST)})
    ...
    def test_compress_to_96(self):
        ...
        df = pd.DataFrame({"MutationType": helpers.MUTATION_LIST * 2, "Value": [1] * len(helpers.MUTATION_LIST) * 2})
        compressed_df = compress_to_96(df)
